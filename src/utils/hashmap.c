...
#define AUTO_DEC_CHECK()                                                                           \
    if (hm->flags & HASHMAP_AUTO_DEC && hm->buckets_x > hm->buckets_x_min &&                       \
 ...
 */
void hashmap_set_opts(hashmap *hm, unsigned int flags, float min_pressure, float max_pressure,
 ...
 */
unsigned int hashmap_size(const hashmap *hm) { return BUCKETS_SIZE(hm->buckets_x); }
 ...
 */
unsigned int hashmap_reserved(const hashmap *hm) { return hm->reserved; }
 ...
 */
void *hashmap_put(hashmap *hm, const void *key, unsigned int keylen, const void *val,
                  unsigned int vallen) {
 ...
 * \return Returns 0 on success, 1 on error (not found).
 */
int hashmap_get(hashmap *hm, const void *key, unsigned int keylen, void **val,
                unsigned int *vallen) {
    unsigned int index = fnv_32a_buf(key, keylen, hm->buckets_x);
...
void hashmap_sdel(hashmap *hm, const char *key) { hashmap_del(hm, key, strlen(key) + 1); }
