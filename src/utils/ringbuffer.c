...
static inline void rb_advance_write(ring_buffer *rb, const size_t len) {
    rb->len += len;
    rb->w_pos += len;
    if(rb->w_pos >= rb->size) {
        rb->w_pos -= rb->size;
...
// Advance read position by len bytes, wrapping if necessary.
static inline void rb_advance_read(ring_buffer *rb, const size_t len) {
    rb->len -= len;
    rb->r_pos += len;
    if(rb->r_pos >= rb->size) {
        rb->r_pos -= rb->size;
...
// Copy len bytes from data into the buffer at write position, handling wraparound.
static void rb_write_data(const ring_buffer *rb, const char *data, const size_t len) {
...
// Copy len bytes from the buffer at read position into data, handling wraparound.
...
size_t rb_write(ring_buffer *rb, const char *data, size_t len) {
    ...
    rb_write_data(rb, data, len);
    rb_advance_write(rb, len);
    return len;
}
