 ...
 *
 * @details Input range 0x00..=0x1F are written as-is, as Open OMF treats these as control characters.
 *          Characters that translate to DOS 0xA9.=0xDF aren't supported, as the OMF fonts don't have useful glyphs
 * there.
 *
 *          At least one of out_cp437 or out_cp437_size should be non-NULL, or this function will assert (and do
 * nothing).
 *
 ...
 *        Exceeding sizeof_out_cp437 will produce CP437_ERROR_OUTPUTBUFFER_TOOSMALL.
 ...
 *        For example, sizeof_utf8 should be 5 for u8"Muß", 2 for `MU`, 2 for `ß` (0xC3 0x9F), and 1 for the `\0`
 * terminator.
 */
 ...
 *        Exceeding sizeof_out_utf8 will produce CP437_ERROR_OUTPUTBUFFER_TOOSMALL.
 ...
 *
 * @details Input range 0x0000..=0x001F are written as-is, as Open OMF treats these as control characters.
 *          Characters that translate to DOS 0xA9.=0xDF aren't supported, as the OMF fonts don't have useful glyphs
 * there.
 *
 * @retval CP437_ERROR_UNKNOWN_CODEPOINT There was a code point we cannot represent in CP437.
 * @retval CP437_SUCCESS Encoding was successful.
 *
