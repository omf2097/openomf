...
typedef enum vga_palette_type {
    VGA_PALETTE_VGA256 = 0, ///< Standard 256 color VGA palette
    VGA_PALETTE_EXT1024,    ///< Extended 1024 color palette
} vga_palette_type;
...
typedef struct vga_palette {
    vga_color colors[1024];
    vga_palette_type type;
} vga_palette;

static_assert(3076 == sizeof(vga_palette), "vga_palette should pack into 3076 bytes");
