...
void reset_targets() {
    ...
    state.fg_target =
        SDL_CreateTexture(state.renderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_TARGET,
...
int video_init(int window_w, int window_h, int fullscreen, int vsync, const char *scaler_name,
               int scale_factor) {
    state.w = window_w;
    state.h = window_h;
    state.fs = fullscreen;
    ...
    // Open window
    state.window = SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, window_w,
    ...
    SDL_RenderSetLogicalSize(state.renderer, NATIVE_W * state.scale_factor,
...
int video_reinit(int window_w, int window_h, int fullscreen, int vsync, const char *scaler_name,
                 int scale_factor) {
...
void video_set_fade(float fade) { state.fade = fade; }

int video_screenshot(image *img) {
    image_create(img, state.w, state.h);
    int ret =
        SDL_RenderReadPixels(state.renderer, NULL, SDL_PIXELFORMAT_ABGR8888, img->data, img->w * 4);
    if (ret != 0) {
...
int video_area_capture(surface *sur, int x, int y, int w, int h) {
    ...
    // Read pixels
    int ret =
        SDL_RenderReadPixels(state.renderer, &r, SDL_PIXELFORMAT_ABGR8888, sur->data, sur->w * 4);
    if (ret != 0) {
        surface_free(sur);
...
palette *video_get_base_palette() { return state.base_palette; }
...
screen_palette *video_get_pal_ref() { return state.screen_palette; }
...
void video_render_bg_separately(bool separate) { state.render_bg_separately = separate; }
...
static void render_sprite_fsot(video_state *state, surface *sur, SDL_Rect *dst,
                               SDL_BlendMode blend_mode, int pal_offset, SDL_RendererFlip flip_mode,
                               uint8_t opacity, color color_mod) {
...
void video_render_sprite_tint(surface *sur, int sx, int sy, color c, int pal_offset) {
    ...
    video_render_sprite_flip_scale_opacity_tint(sur, sx, sy, BLEND_ALPHA, pal_offset, FLIP_NONE,
                                                1.0f, 255, c);
}
...
void video_render_sprite(surface *sur, int sx, int sy, unsigned int rendering_mode,
    ...
    video_render_sprite_flip_scale_opacity(sur, sx, sy, rendering_mode, pal_offset, FLIP_NONE, 1.0f,
                                           255);
}

// Wrapper
void video_render_sprite_flip_scale(surface *sur, int sx, int sy, unsigned int rendering_mode,
                                    int pal_offset, unsigned int flip_mode, float y_percent) {

    video_render_sprite_flip_scale_opacity(sur, sx, sy, rendering_mode, pal_offset, flip_mode,
                                           y_percent, 255);
}

void video_render_sprite_flip_scale_opacity(surface *sur, int sx, int sy,
                                            unsigned int rendering_mode, int pal_offset,
                                            unsigned int flip_mode, float y_percent,
                                            uint8_t opacity) {

    video_render_sprite_flip_scale_opacity_tint(sur, sx, sy, rendering_mode, pal_offset, flip_mode,
                                                y_percent, opacity,
...
void video_render_sprite_flip_scale_opacity_tint(surface *sur, int sx, int sy,
                                                 unsigned int rendering_mode, int pal_offset,
                                                 unsigned int flip_mode, float y_percent,
    ...
    SDL_BlendMode blend_mode =
        (rendering_mode == BLEND_ALPHA) ? SDL_BLENDMODE_BLEND : SDL_BLENDMODE_ADD;

    render_sprite_fsot(&state, sur, &dst, blend_mode, pal_offset, flip, opacity, tint);
}

// Called on every game tick
void video_tick() { tcache_tick(); }
