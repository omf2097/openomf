...
void console_output_scroll_to_end() {
    // iterate the output buffer backward to count up to 16 lines or 480 chars, whichever comes
    // first
...
int console_init() {
    ...
    console_output_addline("");
    console_output_addline(CURSOR_STR
                           "                                   " CURSOR_STR "\n" CURSOR_STR
...
void console_event(game_state *gs, SDL_Event *e) {
    if (e->type == SDL_TEXTINPUT) {
        ...
        /*if ((code >= SDLK_a && code <= SDLK_z) || (code >= SDLK_0 && code <= SDLK_9) || code ==
         * SDLK_SPACE || code == SDLK) {*/
        // SDLK_UP and SDLK_DOWN does not work here
        if (scancode == SDL_SCANCODE_UP) {
            if (con->histpos < HISTORY_MAX &&
                con->histpos < (signed int)(list_size(&con->history) - 1)) {
                con->histpos++;
...
void console_remove_cmd(const char *name) { hashmap_sdel(&con->cmds, name); }

int console_window_is_open() { return con->isopen; }
