...
int console_cmd_har(game_state *gs, int argc, char **argv) {
    ...
    if (argc == 2) {
        ...
        if (strtoint(argv[1], &i)) {
            ...
            if (har_create(obj, game_state_get_scene(gs)->af_data[0], hd, player->har_id,
                           player->pilot_id, 0)) {
                object_free(obj);
                omf_free(obj);
                return 1;
            ...
            game_player_get_har(player)->animation_state.enemy =
                game_player_get_har(game_state_get_player(gs, 1));
            game_player_get_har(game_state_get_player(gs, 1))->animation_state.enemy =
                game_player_get_har(player);
