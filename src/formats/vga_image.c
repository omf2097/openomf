...
int sd_vga_image_encode(sd_vga_image *dst, const sd_rgba_image *src, const palette *pal,
                        int remapping) {
    int ret;
...
int sd_vga_image_decode(sd_rgba_image *dst, const sd_vga_image *src, const palette *pal,
                        int remapping) {
    int ret;
    if (dst == NULL || src == NULL || pal == NULL) {
        return SD_INVALID_INPUT;
