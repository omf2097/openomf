...
int sd_script_set_sprite_at_frame(sd_script *script, int frame_id, int sprite_id) {
    if (script == NULL || frame_id < 0 || frame_id >= script->frame_count || sprite_id < 0 ||
        sprite_id > 25) {
        return SD_INVALID_INPUT;
    }
...
int sd_script_letter_to_frame(char letter) { return (int)(letter - 'A'); }

char sd_script_frame_to_letter(int frame_id) { return (char)(frame_id + 'A'); }
