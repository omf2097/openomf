...
int sd_animation_get_coord_count(sd_animation *ani) { return ani->coord_count; }
...
int sd_animation_get_extra_string_count(sd_animation *anim) { return anim->extra_string_count; }
...
int sd_animation_get_sprite_count(sd_animation *anim) { return anim->sprite_count; }
...
int sd_animation_load(sd_reader *r, sd_animation *ani) {
    ...
    if (ani->extra_string_count > SD_EXTRASTR_COUNT_MAX) {
        DEBUGLOG(
            "Animation has too many extra strings! Expected max %hhu strings, got %hhu strings.",
    ...
    for (int i = 0; i < ani->extra_string_count; i++) {
        ...
        if (size >= SD_EXTRA_STRING_MAX) {
            DEBUGLOG(
                "Animation extra string %d is too long! Expected max %hu bytes, got %hu bytes.", i,
