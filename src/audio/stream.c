...
void stream_set_finished(audio_stream *stream) { stream->status = STREAM_STATUS_FINISHED; }
...
int stream_get_status(audio_stream *stream) { return stream->status; }

void stream_set_userdata(audio_stream *stream, void *userdata) { stream->userdata = userdata; }

void *stream_get_userdata(audio_stream *stream) { return stream->userdata; }

void stream_set_update_cb(audio_stream *stream, stream_update_cb cbfunc) {
    stream->update = cbfunc;
}

void stream_set_close_cb(audio_stream *stream, stream_close_cb cbfunc) { stream->close = cbfunc; }

void stream_set_play_cb(audio_stream *stream, stream_play_cb cbfunc) { stream->play = cbfunc; }

void stream_set_stop_cb(audio_stream *stream, stream_stop_cb cbfunc) { stream->stop = cbfunc; }

void stream_set_apply_cb(audio_stream *stream, stream_apply_cb cbfunc) { stream->apply = cbfunc; }
