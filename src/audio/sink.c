...
unsigned int gid_gen() { return _sink_global_id++; }
...
void sink_play(audio_sink *sink, audio_source *src, int id, float volume, float panning,
               float pitch) {
...
void sink_set_stream_panning(audio_sink *sink, int sid, float panning) {
    if (panning < PANNING_MIN || panning > PANNING_MAX)
        return;
    audio_stream *s = sink_get_stream(sink, sid);
    s->panning = panning;
    stream_apply(s);
...
float sink_get_stream_pitch(audio_sink *sink, int sid) { return sink_get_stream(sink, sid)->pitch; }

void sink_set_userdata(audio_sink *sink, void *userdata) { sink->userdata = userdata; }

void *sink_get_userdata(audio_sink *sink) { return sink->userdata; }

void sink_set_close_cb(audio_sink *sink, sink_close_cb cbfunc) { sink->close = cbfunc; }
