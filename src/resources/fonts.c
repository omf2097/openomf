...
void fonts_set_font(font *new_font, font_size font_sz) {
    switch(font_sz) {
        case FONT_BIG:
            memcpy(&font_large, new_font, sizeof(font));
            break;
        case FONT_SMALL:
            memcpy(&font_small, new_font, sizeof(font));
            break;
        case FONT_NET1:
            memcpy(&font_net1, new_font, sizeof(font));
            break;
        case FONT_NET2:
            memcpy(&font_net2, new_font, sizeof(font));
            break;
        default:
            return;
    }
