...
void bk_create(bk *b, void *src) {
    ...
    // Copy VGA image
    surface_create_from_data(&b->background, SURFACE_TYPE_PALETTE, sdbk->background->w,
                             sdbk->background->h, sdbk->background->data);
...
palette *bk_get_palette(bk *b, int id) { return vector_get(&b->palettes, id); }

char *bk_get_stl(bk *b) { return b->sound_translation_table; }
