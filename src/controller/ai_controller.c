 ...
 */
bool roll_chance(int roll_x) { return roll_x <= 1 ? true : rand_int(roll_x) == 1; }
...
bool likes_tactic(const controller *ctrl, int tactic_type) {
    ...
    switch (tactic_type) {
    case TACTIC_SHOOT:
        if (har_has_projectiles(h->id) && roll_pref(pilot->att_sniper) &&
    ...
    case TACTIC_QUICK:
        if (enemy_range > RANGE_CRAMPED && enemy_range < RANGE_FAR &&
            ((roll_pref(pilot->att_sniper) && roll_chance(3)) ||
             (roll_pref(pilot->att_hyper) && roll_chance(6)) ||
    ...
    case TACTIC_COUNTER:
        ...
        break;
    case TACTIC_ESCAPE:
        if ((roll_pref(pilot->att_jump) && roll_chance(3)) ||
            (roll_pref(pilot->att_def) && roll_chance(5))) {
            return true;
        }
        break;
    case TACTIC_FLY:
    ...
    case TACTIC_PUSH:
        if ((enemy_range <= RANGE_CLOSE ||
            ...
            ((har_has_push(h->id) && smart_usually(a)) &&
             ((roll_pref(pilot->att_hyper) && roll_chance(2)) ||
              (roll_pref(pilot->att_def) && roll_chance(4)) || (wall_close && roll_chance(5))))) {
            return true;
        }
        break;
    case TACTIC_TRIP:
        if (enemy_range <= RANGE_MID && ((roll_pref(pilot->att_def) && roll_chance(4)) ||
                                         (roll_pref(pilot->att_sniper) && roll_chance(6)))) {
            return true;
        }
        break;
    case TACTIC_SPAM:
        if ((enemy_close || dumb_usually(a)) && (wall_close || roll_chance(6)) &&
            roll_pref(pilot->att_normal)) {
            return true;
        }
        break;
    }
...
void queue_tactic(controller *ctrl, int tactic_type) {
    ...
    case TACTIC_CLOSE:
        ...
        } else if ((tactic_type == TACTIC_CLOSE ||
 ...
 * \return Void.
 */
void reset_act_timer(ai *a) { a->act_timer = BASE_ACT_TIMER - (a->difficulty * 2) - rand_int(3); }
...
int ai_har_event(controller *ctrl, har_event event) {
    ...
    if (has_queued_tactic) {
        ...
        case HAR_EVENT_BLOCK_PROJECTILE:
            if (a->tactic->tactic_type != TACTIC_COUNTER &&
                a->tactic->tactic_type != TACTIC_TURTLE && a->tactic->tactic_type != TACTIC_TRIP &&
                a->tactic->tactic_type != TACTIC_PUSH && a->tactic->tactic_type != TACTIC_SPAM &&
    ...
    case HAR_EVENT_LAND_HIT_PROJECTILE:
        ...
        if (event.type == HAR_EVENT_LAND_HIT_PROJECTILE) {
            ...
            chain_consider_tactics(ctrl,
                                   (int[]){TACTIC_FLY, TACTIC_TURTLE, TACTIC_CLOSE, TACTIC_SHOOT});
        } else {
            // we hit with a HAR attack
            chain_consider_tactics(ctrl,
                                   (int[]){TACTIC_QUICK, TACTIC_TRIP, TACTIC_GRAB, TACTIC_PUSH,
    ...
    case HAR_EVENT_ENEMY_BLOCK_PROJECTILE:
        ...
        if (!a->blocked) {
            ...
            if (event.type == HAR_EVENT_ENEMY_BLOCK_PROJECTILE) {
                ...
                chain_consider_tactics(ctrl, (int[]){TACTIC_FLY, TACTIC_ESCAPE, TACTIC_TURTLE,
                                                     TACTIC_CLOSE, TACTIC_SHOOT});
            } else {
                // enemy blocked our HAR attack
                chain_consider_tactics(ctrl, (int[]){TACTIC_GRAB, TACTIC_TRIP, TACTIC_PUSH,
                                                     TACTIC_COUNTER, TACTIC_TURTLE, TACTIC_ESCAPE,
                                                     TACTIC_FLY, TACTIC_QUICK, TACTIC_SPAM});
            }
        }
    ...
    case HAR_EVENT_BLOCK_PROJECTILE:
        ...
        if (event.type == HAR_EVENT_BLOCK_PROJECTILE) {
            ...
            chain_consider_tactics(ctrl,
                                   (int[]){TACTIC_FLY, TACTIC_SHOOT, TACTIC_CLOSE, TACTIC_TURTLE});
        } else {
            // we blocked a HAR attack
            chain_consider_tactics(ctrl,
                                   (int[]){TACTIC_TRIP, TACTIC_PUSH, TACTIC_TURTLE, TACTIC_GRAB,
    ...
    case HAR_EVENT_LAND:
        ...
        if (has_queued_tactic && a->tactic->attack_on == HAR_EVENT_LAND &&
            ...
            if (!has_queued_tactic && smart_usually(a)) {
                chain_consider_tactics(ctrl, (int[]){TACTIC_TRIP, TACTIC_SHOOT, TACTIC_TURTLE,
                                                     TACTIC_QUICK, TACTIC_GRAB, TACTIC_PUSH,
                                                     TACTIC_COUNTER, TACTIC_CLOSE});
            }
        }
    ...
    case HAR_EVENT_HIT_WALL:
        ...
        chain_consider_tactics(ctrl,
                               (int[]){TACTIC_SHOOT, TACTIC_PUSH, TACTIC_TURTLE, TACTIC_TRIP,
    ...
    case HAR_EVENT_TAKE_HIT_PROJECTILE:
        ...
        if (event.move->category == CAT_THROW || event.move->category == CAT_CLOSE) {
            ...
            // aggressive tactics
            chain_consider_tactics(ctrl,
                                   (int[]){TACTIC_CLOSE, TACTIC_FLY, TACTIC_SHOOT, TACTIC_GRAB});
        } else {
            // defensive tactics
            chain_consider_tactics(ctrl,
                                   (int[]){TACTIC_COUNTER, TACTIC_TURTLE, TACTIC_ESCAPE,
                                           TACTIC_PUSH, TACTIC_TRIP, TACTIC_QUICK, TACTIC_SPAM});
        }
    ...
    case HAR_EVENT_RECOVER:
        ...
        chain_consider_tactics(ctrl,
                               (int[]){TACTIC_SHOOT, TACTIC_COUNTER, TACTIC_TURTLE, TACTIC_ESCAPE});
        break;
    case HAR_EVENT_ENEMY_STUN:
            ...
            if (is_valid_move(move, h, true)) {
                ...
                if (highest_damage) {
                    ...
                    if (learning_moment(a) && ms->min_hit_dist != -1) {
                        if (ms->last_dist < ms->max_hit_dist + 5 &&
                            ms->last_dist > ms->min_hit_dist + 5) {
                            value += 2;
    ...
    while ((o_tmp = iter_next(&it)) != NULL) {
        ...
        if (o_prj->cur_sprite && (smart_usually(a) || remember_shooting)) {
            ...
            if (object_get_direction(o_prj) == OBJECT_FACE_LEFT) {
                pos_prj.x =
                    object_get_pos(o_prj).x + ((o_prj->cur_sprite->pos.x * -1) - size_prj.x);
            }
            if (fabsf(pos_prj.x - o->pos.x) < 120) {
                a->cur_act = (o->direction == OBJECT_FACE_RIGHT ? ACT_DOWN | ACT_LEFT
                                                                : ACT_DOWN | ACT_RIGHT);
                controller_cmd(ctrl, a->cur_act, ev);
                return 1;
            }
...
void handle_movement(controller *ctrl, ctrl_event **ev) {
    ...
    if (a->act_timer <= 0 && (roll_chance(BASE_ACT_CHANCE) || diff_scale(a))) {
        ...
        } else if ((h->id == HAR_FLAIL || h->id == HAR_THORN || h->id == HAR_NOVA) &&
                   smart_usually(a)) {
...
bool attempt_attack(controller *ctrl, bool highest_damage) {
    ...
    bool in_attempt_range =
        (enemy_range <= RANGE_CLOSE || (enemy_range == RANGE_MID && dumb_sometimes(a)));
    ...
    for (int i = 0; i < 70; i++) {
        ...
        if ((move = af_get_move(h->af_data, i))) {
            ...
            if (is_valid_move(move, h, false)) {
                ...
                if (!in_attempt_range &&
                    (move->category == CAT_BASIC || move->category == CAT_LOW ||
...
bool attempt_charge_attack(controller *ctrl, ctrl_event **ev) {
    ...
    case HAR_JAGUAR:
        ...
        if (enemy_range >= RANGE_MID && roll_pref(a->pilot->ap_special) && diff_scale(a)) {
            ...
            chain_controller_cmd(
                ...
                (int[]){(o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT : ACT_RIGHT),
                        (o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT | ACT_DOWN
                                                           : ACT_RIGHT | ACT_DOWN)},
                ev);
        }
        // Jaguar Leap : D,F+P
        chain_controller_cmd(ctrl,
                             (int[]){ACT_DOWN,
                                     (o->direction == OBJECT_FACE_RIGHT ? ACT_DOWN | ACT_RIGHT
                                                                        : ACT_DOWN | ACT_LEFT),
    ...
    case HAR_SHADOW:
        ...
        // Shadow Grab       : D,D+P
        chain_controller_cmd(
            ctrl, (int[]){ACT_DOWN, ACT_STOP, ACT_DOWN, ACT_DOWN | ACT_PUNCH, ACT_PUNCH}, ev);
        break;
    case HAR_KATANA:
        if (roll_chance(2) && roll_pref(a->pilot->ap_low)) {
            ...
            chain_controller_cmd(ctrl,
                                 (int[]){ACT_DOWN,
                                         (o->direction == OBJECT_FACE_RIGHT ? ACT_DOWN | ACT_LEFT
                                                                            : ACT_DOWN | ACT_RIGHT),
                                         ACT_KICK},
                                 ev);
        } else {
            if (enemy_range >= RANGE_MID && roll_chance(2)) {
                ...
                chain_controller_cmd(
                    ...
                    (int[]){ACT_DOWN, (o->direction == OBJECT_FACE_RIGHT ? ACT_RIGHT : ACT_LEFT),
                            (o->direction == OBJECT_FACE_RIGHT ? ACT_RIGHT | ACT_KICK
                                                               : ACT_LEFT | ACT_KICK),
                            (o->direction == OBJECT_FACE_RIGHT ? ACT_RIGHT : ACT_LEFT)},
                    ev);
            } else {
                // DEBUG("\e[35mKatana move:\e[0m Rising Blade ");
                if (enemy_range >= RANGE_CLOSE && roll_pref(a->pilot->ap_special) &&
                    diff_scale(a)) {
                ...
                chain_controller_cmd(
                    ...
                    (int[]){ACT_DOWN,
                            (o->direction == OBJECT_FACE_RIGHT ? ACT_DOWN | ACT_RIGHT
                                                               : ACT_DOWN | ACT_LEFT),
                            (o->direction == OBJECT_FACE_RIGHT ? ACT_RIGHT : ACT_LEFT),
                            (o->direction == OBJECT_FACE_RIGHT ? ACT_RIGHT | ACT_PUNCH
                                                               : ACT_LEFT | ACT_PUNCH),
                            ACT_PUNCH},
                    ev);
            }
    ...
    case HAR_FLAIL:
        ...
        if (enemy_range > RANGE_MID && roll_pref(a->pilot->ap_special) && diff_scale(a)) {
            ...
            chain_controller_cmd(
                ctrl,
                (int[]){ACT_DOWN, (o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT | ACT_DOWN
                                                                     : ACT_RIGHT | ACT_DOWN)},
                ev);
        }
        ...
        chain_controller_cmd(ctrl,
                             (int[]){(o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT : ACT_RIGHT),
                                     ...
                                     (o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT | ACT_PUNCH
                                                                        : ACT_RIGHT | ACT_PUNCH),
                                     ACT_PUNCH},
                             ev);
        break;
    ...
    case HAR_PYROS:
        ...
        if (enemy_range > RANGE_MID && roll_pref(a->pilot->ap_special) && diff_scale(a)) {
            ...
            chain_controller_cmd(
                ctrl, (int[]){(o->direction == OBJECT_FACE_RIGHT ? ACT_RIGHT : ACT_LEFT), ACT_STOP},
                ev);
        }
        // Super Thrust : F,F+P
        chain_controller_cmd(ctrl,
    ...
    case HAR_ELECTRA:
        ...
        if (enemy_range >= RANGE_MID && roll_pref(a->pilot->ap_special) && diff_scale(a)) {
            // Super R.T. : B,D,F,F+P
            chain_controller_cmd(
                ctrl, (int[]){(o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT : ACT_RIGHT), ACT_DOWN},
                ev);
        }
    ...
    case HAR_CHRONOS:
        if (enemy_range >= RANGE_MID && roll_pref(a->pilot->ap_special) && diff_scale(a)) {
            ...
            chain_consider_tactics(
                ctrl, (int[]){TACTIC_GRAB, TACTIC_PUSH, TACTIC_SHOOT, TACTIC_SPAM, TACTIC_TRIP});
        } else {
    ...
    case HAR_SHREDDER:
        if (enemy_range > RANGE_MID && roll_pref(a->pilot->att_jump) && diff_scale(a)) {
            ...
            // Flip Kick : D,D+K
            chain_controller_cmd(
                ctrl, (int[]){ACT_DOWN, ACT_STOP, ACT_DOWN, ACT_DOWN | ACT_KICK, ACT_KICK}, ev);
        } else {
...
bool attempt_push_attack(controller *ctrl, ctrl_event **ev) {
    ...
    case HAR_JAGUAR:
        ...
        chain_controller_cmd(ctrl,
                             (int[]){(o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT : ACT_RIGHT),
                                     (o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT | ACT_KICK
                                                                        : ACT_RIGHT | ACT_KICK),
                                     ACT_KICK},
                             ev);
        break;
    ...
    case HAR_NOVA:
        if (diff_scale(a)) {
            ...
            // Heavy Kick : B+K
            chain_controller_cmd(ctrl,
                                 (int[]){(o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT | ACT_KICK
                                                                            : ACT_RIGHT | ACT_KICK),
                                         ACT_KICK},
                                 ev);
        }
...
bool attempt_trip_attack(controller *ctrl, ctrl_event **ev) {
    ...
    chain_controller_cmd(
        ...
        (int[]){ACT_DOWN,
                (o->direction == OBJECT_FACE_RIGHT ? ACT_DOWN | ACT_LEFT : ACT_DOWN | ACT_RIGHT),
                (o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT | ACT_KICK : ACT_RIGHT | ACT_KICK),
                ACT_KICK},
        ev);
...
bool attempt_projectile_attack(controller *ctrl, ctrl_event **ev) {
    ...
    case HAR_SHREDDER: // Flying Hands : D, B+P
        ...
        chain_controller_cmd(ctrl,
                             (int[]){(o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT | ACT_PUNCH
                                                                        : ACT_RIGHT | ACT_PUNCH),
                                     ACT_PUNCH},
                             ev);
        break;
    ...
    case HAR_NOVA:
        ...
        if (roll_chance(3)) {
            ...
            chain_controller_cmd(
                ctrl,
                (int[]){(o->direction == OBJECT_FACE_RIGHT ? ACT_DOWN | ACT_LEFT
                                                           : ACT_DOWN | ACT_RIGHT),
                        (o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT : ACT_RIGHT)},
                ev);
        } else {
            ...
            chain_controller_cmd(
                ctrl,
                (int[]){(o->direction == OBJECT_FACE_RIGHT ? ACT_DOWN | ACT_RIGHT
                                                           : ACT_DOWN | ACT_LEFT),
                        (o->direction == OBJECT_FACE_RIGHT ? ACT_RIGHT : ACT_LEFT)},
                ev);
        }
        ...
        case MOVE_AVOID:
            if (enemy_range == RANGE_FAR) {
                ...
                if (enemy_range == RANGE_CRAMPED || !roll_pref(a->pilot->pref_jump)) {
                    ...
                    // jump away
                    a->cur_act =
                        (o->direction == OBJECT_FACE_RIGHT ? ACT_LEFT : ACT_RIGHT) | ACT_UP;
                }
        ...
        case MOVE_HIGH_JUMP:
            if (enemy_range > RANGE_CRAMPED) {
                if ((enemy_range == RANGE_FAR && smart_usually(a)) ||
                    tactic->move_type == MOVE_HIGH_JUMP) {
                    // do high jump
                    controller_cmd(ctrl, ACT_DOWN, ev);
                }
                // jump closer
...
int ai_controller_poll(controller *ctrl, ctrl_event **ev) {
    ...
    bool can_move =
        (h->state == STATE_STANDING || h->state == STATE_WALKTO || h->state == STATE_WALKFROM ||
    ...
    if (a->last_move_id > 0 && (roll_chance(10) && diff_scale(a)) && a->tactic->tactic_type == 0 &&
        can_move) {
        // DEBUG("\e[35mAttempt to queue random tactic[0m");
        chain_consider_tactics(ctrl, (int[]){TACTIC_SHOOT, TACTIC_CLOSE, TACTIC_FLY, TACTIC_PUSH,
                                             TACTIC_TRIP, TACTIC_GRAB, TACTIC_QUICK});
    }
