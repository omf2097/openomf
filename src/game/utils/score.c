...
typedef struct score_text_t {
    char *text;
    float
        position; // Position of text between middle of screen and (x,y). 1.0 at middle, 0.0 at end
    vec2i start;
    int points;
    int age;
} score_text;
...
void chr_score_set_difficulty(chr_score *score, int difficulty) { score->difficulty = difficulty; }
...
void chr_score_reset_wins(chr_score *score) { score->wins = 0; }
...
unsigned int chr_score_get_num_texts(chr_score *score) { return list_size(&score->texts); }

int chr_score_onscreen(chr_score *score) { return list_size(&score->texts) > 0; }
...
void chr_score_render(chr_score *score) {
    ...
    if (score->direction == OBJECT_FACE_RIGHT) {
        font_render_shadowed(&font_small, tmp, score->x, score->y, TEXT_COLOR,
    ...
    while ((t = iter_next(&it)) != NULL) {
        ...
        if (score->direction == OBJECT_FACE_LEFT) {
            pos = interpolate(vec2i_create(score->x - (strlen(t->text) * font_small.w), score->y),
                              t->start, t->position);
        }
        font_render_shadowed(&font_small, t->text, pos.x, pos.y, TEXT_COLOR,
                             TEXT_SHADOW_RIGHT | TEXT_SHADOW_BOTTOM);
        lastage = t->age;
    }
}
...
void chr_score_scrap(chr_score *score) { score->scrap = 1; }

void chr_score_destruction(chr_score *score) { score->destruction = 1; }
...
void chr_score_clear_done(chr_score *score) { score->done = 0; }
