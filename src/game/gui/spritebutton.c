...
static void spritebutton_render(component *c) {
    ...
    if (c->is_disabled) {
        video_render_sprite_flip_scale_opacity_tint(sb->img, c->x, c->y, BLEND_ALPHA, 0, 0, 1.0,
                                                    opacity, color_create(128, 128, 128, 255));
    } else if (sb->active > 0) {
...
component *spritebutton_create(const text_settings *tconf, const char *text, surface *img,
                               int disabled, spritebutton_click_cb cb, void *userdata) {
