...
static void textinput_render(component *c) {
    ...
    if(component_is_selected(c)) {
        int i = (ti->ticks / 10) % 16;
        if(i > 8) {
            i = 16 - i;
        }
        text_set_color(ti->text, 216 + i);
    } else if(component_is_disabled(c)) {
        text_set_color(ti->text, theme->text.disabled_color);
    }

    text_draw(ti->text, c->x + 3, c->y + 1);
}
...
static int textinput_action(component *c, int action) {
    textinput *ti = widget_get_obj(c);
    str *buf = text_get_str(ti->text);
    char cursor_char = str_at(buf, ti->pos);
    char new_char;
    switch(action) {
        case ACT_RIGHT:
            if(cursor_char == '\0' && ti->pos >= (int)str_size(buf)) {
                str_append_c(buf, " ");
            }
            ti->pos = min2(ti->max_chars - 1, ti->pos + 1);
            str_truncate(buf, ti->max_chars);
            return 0;
            break;
        case ACT_LEFT:
        ...
        case ACT_UP:
            ...
            if(ti->pos >= (int)str_size(buf)) {
                str_append_buf(buf, &new_char, 1);
            } else {
                str_set_at(buf, ti->pos, new_char);
            }
            str_truncate(buf, ti->max_chars);
            return 0;
            break;
...
static int textinput_event(component *c, SDL_Event *e) {
    ...
    if(e->type == SDL_TEXTINPUT) {
        str_insert_at(buf, ti->pos, e->text.text[0]);
        str_truncate(buf, ti->max_chars);
        ti->pos = min2(ti->max_chars - 1, ti->pos + 1);
        return 0;
...
static void textinput_init(component *c, const gui_theme *theme) {
    ...
    text_set_margin(ti->text, (text_margin){1, 1, 2, 2});
...
static void textinput_layout(component *c, int x, int y, int w, int h) {
    textinput *ti = widget_get_obj(c);
    text_set_bounding_box(ti->text, w - 6, h - (ti->bg_enabled ? 2 : 0));
    text_set_horizontal_align(ti->text, ti->text_horizontal_align);
    text_set_vertical_align(ti->text, ALIGN_TEXT_MIDDLE);
    text_generate_layout(ti->text);
    if(ti->bg_enabled) {
        image img;
        image_create(&img, w, h);
        image_clear(&img, 0);
        image_rect(&img, 0, 0, w - 6, h, COLOR_MENU_BORDER);
...
component *textinput_create(int max_chars, const char *help, const char *initial_value) {
    ...
    ti->text_horizontal_align = ALIGN_TEXT_LEFT;
    ti->text = text_create_with_size(FONT_SMALL, TEXT_BBOX_MAX, TEXT_BBOX_MAX);
    text_set_from_c(ti->text, initial_value);

    component_set_help_text(c, help);

    // Truncate initial value, if needed.
    str *buf = text_get_str(ti->text);
    str_truncate(buf, ti->max_chars);
    ti->pos = min2(str_size(buf), ti->max_chars);
