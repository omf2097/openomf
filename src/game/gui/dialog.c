...
void dialog_create(dialog *dlg, dialog_style style, const char *text, int x, int y) {
    ...
    font_get_wrapped_size_shadowed(&font_small, dlg->text, MAX_WIDTH,
                                   TEXT_SHADOW_RIGHT | TEXT_SHADOW_BOTTOM, &w, &h);
...
void dialog_show(dialog *dlg, int visible) { dlg->visible = visible; }

int dialog_is_visible(dialog *dlg) { return dlg->visible; }
...
void dialog_render(dialog *dlg) {
    ...
    font_render_wrapped_shadowed(&font_small, dlg->text, dlg->x + 15, dlg->y + 3, MAX_WIDTH,
