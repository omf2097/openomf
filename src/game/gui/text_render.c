...
void text_render_char(const text_settings *settings, int x, int y, char ch) {
    ...
    if (settings->shadow & TEXT_SHADOW_RIGHT)
        video_render_sprite_flip_scale_opacity_tint(*sur, x + 1, y, BLEND_ALPHA, 0, FLIP_NONE, 1.0f,
                                                    of * 80, settings->cforeground);
    if (settings->shadow & TEXT_SHADOW_LEFT)
        video_render_sprite_flip_scale_opacity_tint(*sur, x - 1, y, BLEND_ALPHA, 0, FLIP_NONE, 1.0f,
                                                    of * 80, settings->cforeground);
    if (settings->shadow & TEXT_SHADOW_BOTTOM)
        video_render_sprite_flip_scale_opacity_tint(*sur, x, y + 1, BLEND_ALPHA, 0, FLIP_NONE, 1.0f,
                                                    of * 80, settings->cforeground);
    ...
    video_render_sprite_flip_scale_opacity_tint(*sur, x, y, BLEND_ALPHA, 0, FLIP_NONE, 1,
...
int text_char_width(const text_settings *settings) { return (settings->font == FONT_BIG) ? 8 : 6; }
...
void font_render_len_shadowed(const font *font, const char *text, int len, int x, int y, color c,
                              int shadow_flags) {
    int pos_x = x;
    for (int i = 0; i < len; i++) {
...
void font_render_shadowed(const font *font, const char *text, int x, int y, color c,
                          int shadow_flags) {
    int len = strlen(text);
...
void font_render_wrapped_internal(const font *font, const char *text, int x, int y, int max_w,
                                  color c, int shadow_flags, int only_size, int *out_w,
                                  int *out_h) {
    int len = strlen(text);
    int has_newline = 0;
    for (int i = 0; i < len; i++) {
        ...
        *out_h = 0;
        while (start != end) {
                ...
                if (*stop == 0) {
                    ...
                    if (stop - start > maxlen) {
                        ...
                        if (tmpstop - start > maxlen) {
                            // this line cannot not be word-wrapped because it contains a word that
                            // exceeds maxlen, we'll let it pass
                            stop--;
                            is_last_line = 1;
                        } else {
            ...
            if (!only_size) {
                ...
                font_render_len_shadowed(font, start, linelen + (is_last_line ? 1 : 0), x + xoff,
                                         y + yoff, c, shadow_flags);
            }
...
void font_render_wrapped_shadowed(const font *font, const char *text, int x, int y, int w, color c,
                                  int shadow_flags) {
    int tmp;
...
void font_get_wrapped_size_shadowed(const font *font, const char *text, int max_w, int shadow_flag,
                                    int *out_w, int *out_h) {
    static color c = {0};
