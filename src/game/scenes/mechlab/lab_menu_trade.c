...
void lab_menu_trade(component *c, void *userdata) {
    ...
    if(trade_value == har_value) {
        snprintf(tmp, 100, lang_get(LangTradeEquitable), lang_get_offset(LangRobot, p1->chr->pilot.har_id),
                 lang_get_offset(LangRobot, p1->pilot->har_id));
    } else if(trade_value > har_value) {
        char price[15];
        snprintf(price, 15, "$ %dK", trade_value - har_value);
        snprintf(tmp, 100, lang_get(LangTradeGainCash), lang_get_offset(LangRobot, p1->chr->pilot.har_id),
                 lang_get_offset(LangRobot, p1->pilot->har_id), price);
    } else if(trade_value + p1->pilot->money > har_value) {
        char price[15];
        snprintf(price, 15, "$ %dK", har_value - trade_value);
        snprintf(tmp, 100, lang_get(LangTradeSpendCash), lang_get_offset(LangRobot, p1->chr->pilot.har_id), price,
                 lang_get_offset(LangRobot, p1->pilot->har_id));
    }
