...
void render_highlights(scene *scene) {
    ...
    if (player2->selectable && local->row_a == local->row_b && local->column_a == local->column_b) {
        video_render_sprite_tint(&local->select_hilight, 11 + (62 * local->column_a),
                                 115 + (42 * local->row_a),
        ...
        if (player2->selectable) {
            ...
            video_render_sprite_tint(&local->select_hilight, 11 + (62 * local->column_b),
                                     115 + (42 * local->row_b), color_create(0, 0, trans, 0), 0);
        }
        if (local->done_a) {
            trans = 250;
        } else {
        ...
        video_render_sprite_tint(&local->select_hilight, 11 + (62 * local->column_a),
                                 115 + (42 * local->row_a), color_create(trans, 0, 0, 0), 0);
    }
}
...
void melee_render(scene *scene) {
    ...
    if (local->selection == 0) {
        ...
        // player bio
        font_render_wrapped_shadowed(&font_small, lang_get(135 + current_a), 4, 66, 152,
        ...
        if (player2->selectable) {
            ...
            // player bio
            font_render_wrapped_shadowed(&font_small, lang_get(135 + current_b),
                                         320 - local->bleh.w + 4, 66, 152, COLOR_GREEN,
                                         TEXT_SHADOW_RIGHT | TEXT_SHADOW_BOTTOM);
            // player stats
            font_render_shadowed(&font_small, lang_get(216), 320 - 66 - local->feh.w + 27, 4,
                                 COLOR_GREEN, TEXT_SHADOW_RIGHT | TEXT_SHADOW_BOTTOM);
            font_render_shadowed(&font_small, lang_get(217), 320 - 66 - local->feh.w + 19, 22,
                                 COLOR_GREEN, TEXT_SHADOW_RIGHT | TEXT_SHADOW_BOTTOM);
            font_render_shadowed(&font_small, lang_get(218), 320 - 66 - local->feh.w + 12, 40,
                                 COLOR_GREEN, TEXT_SHADOW_RIGHT | TEXT_SHADOW_BOTTOM);
    ...
    if (local->selection == 0) {
        ...
        if (player2->selectable) {
            // player 2 name
            font_render_wrapped_shadowed(&font_small, lang_get(20 + current_b), 320 - 66, 52, 66,
        ...
        // player 1 name
        font_render_wrapped_shadowed(&font_small, lang_get(20 + local->pilot_id_a), 0, 52, 66,
                                     COLOR_BLACK, TEXT_SHADOW_TOP | TEXT_SHADOW_LEFT);

        if (player2->selectable) {
            // player 2 name
            font_render_wrapped_shadowed(&font_small, lang_get(20 + local->pilot_id_b), 320 - 66,
            ...
            // render HAR name
            font_render_wrapped_shadowed(&font_small,
                                         har_get_name(5 * local->row_a + local->column_a), 130, 107,
                                         66, COLOR_BLACK, TEXT_SHADOW_TOP | TEXT_SHADOW_LEFT);
        }
    }
    ...
    if (player2->selectable) {
        ...
        font_render_shadowed(&font_small, winstext, 8, 107, COLOR_BLACK,
                             TEXT_SHADOW_TOP | TEXT_SHADOW_LEFT);
        snprintf(winstext, 48, "Wins: %d", s2->wins);
        font_render_shadowed(&font_small, winstext, 312 - (strlen(winstext) * font_small.w), 107,
                             COLOR_BLACK, TEXT_SHADOW_TOP | TEXT_SHADOW_LEFT);
    }
}
...
int melee_create(scene *scene) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        object_create(&local->harportraits_player1[i], scene->gs, vec2i_create(0, 0),
                      vec2f_create(0, 0));
        ...
        if (player2->selectable) {
            ...
            mask_sprite(spr->data, 62 * col, 42 * row, 51, 36);
            ani = create_animation_from_single(spr, spr->pos);
            object_create(&local->harportraits_player2[i], scene->gs, vec2i_create(0, 0),
                          vec2f_create(0, 0));
            ...
            ani = &bk_get_info(&scene->bk_data, 18 + i)->ani;
            object_create(&local->har_player2[i], scene->gs, vec2i_create(210, 95),
                          vec2f_create(0, 0));
    ...
    ani = create_animation_from_single(spr, spr->pos);
    object_create(&local->unselected_har_portraits, scene->gs, vec2i_create(0, 0),
                  vec2f_create(0, 0));
