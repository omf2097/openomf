...
void mechlab_enter_trnselect_menu(scene *scene) {
    ...
    component *menu =
        lab_menu_select_create(scene, lab_dash_trnselect_select, &local->tw, lab_dash_trnselect_left, &local->tw,
                               lab_dash_trnselect_right, &local->tw, lang_get(LangSelectTournament), true);
...
component *mechlab_chrload_menu_create(scene *scene) {
    mechlab_local *local = scene_get_userdata(scene);
    component *menu =
        lab_menu_select_create(scene, lab_dash_main_chr_load, &local->dw, lab_dash_main_chr_left, &local->dw,
                               lab_dash_main_chr_right, &local->dw, lang_get(LangSelectChrToLoad), true);
...
component *mechlab_chrdelete_menu_create(scene *scene) {
    mechlab_local *local = scene_get_userdata(scene);
    component *menu =
        lab_menu_select_create(scene, lab_dash_main_chr_delete, &local->dw, lab_dash_main_chr_left, &local->dw,
                               lab_dash_main_chr_right, &local->dw, lang_get(LangSelectChrToDelete), true);
    trnmenu_set_submenu_init_cb(menu, lab_dash_main_chr_init);
    trnmenu_set_submenu_done_cb(menu, lab_dash_main_chr_done);
    trnmenu_set_userdata(menu, &local->dw);
    return menu;
...
component *mechlab_sim_menu_create(scene *scene) {
    ...
    component *menu = lab_menu_select_create(scene, lab_dash_main_chr_delete, &local->dw, lab_dash_sim_left, &local->dw,
                                             lab_dash_sim_right, &local->dw, lang_get(LangSelectOpponent), true);
...
void mechlab_tick(scene *scene, int paused) {
    ...
    if(trnmenu_is_finished(root)) {
        ...
        if(local->dashtype == DASHBOARD_NEW_PLAYER) {
            ...
            snprintf(select_photo, sizeof(select_photo), lang_get(LangSelectPhotoForChr), player1->pilot->name);
