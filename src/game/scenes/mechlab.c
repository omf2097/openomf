...
void mechlab_select_dashboard(scene *scene, mechlab_local *local, dashboard_type type) {
    ...
    case DASHBOARD_SELECT_NEW_PIC:
        // Dashboard widgets struct is filled with pointer to the necessary components for easy
        // access
        ...
        do {
            if (i->type == EVENT_TYPE_ACTION) {
                ...
                if (local->dashtype == DASHBOARD_NEW) {
                    ...
                    } else if (i->event_data.action == ACT_KICK ||
                               i->event_data.action == ACT_PUNCH) {
                        mechlab_select_dashboard(scene, local, DASHBOARD_SELECT_NEW_PIC);
                        trnmenu_finish(guiframe_get_root(
                            local->frame)); // This will trigger exception case in mechlab_tick
                    } else {
...
int mechlab_create(scene *scene) {
    ...
    if (last_name != NULL) {
        ...
        if (ret != SD_SUCCESS) {
            PERROR("Could not load saved game for playername '%s': %s!", last_name,
                   sd_get_error(ret));
            last_name = NULL;
        } else {
