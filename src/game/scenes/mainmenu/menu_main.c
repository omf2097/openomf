...
component *menu_main_create(scene *s) {
    ...
    menu_attach(menu,
                textbutton_create(&tconf, "TOURNAMENT PLAY", COM_ENABLED, mainmenu_mechlab, s));
    component *net =
        textbutton_create(&tconf, "NETWORK PLAY", COM_ENABLED, mainmenu_enter_network, s);
    widget_set_id(net, NETWORK_BUTTON_ID);
    menu_attach(menu, net);
    menu_attach(menu, textbutton_create(&tconf, "CONFIGURATION", COM_ENABLED,
                                        mainmenu_enter_configuration, s));
    menu_attach(menu,
                textbutton_create(&tconf, "GAMEPLAY", COM_ENABLED, mainmenu_enter_gameplay, s));
