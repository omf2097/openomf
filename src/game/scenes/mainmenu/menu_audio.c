...
void menu_audio_reset_freqs(audio_menu_data *local, int use_settings) {
    ...
    for (int i = 0; freqs[i].name != 0; i++) {
        ...
        int use = (use_settings) ? (settings_get()->sound.music_frequency == freqs[i].freq)
                                 : freqs[i].is_default;
        if (use) {
...
component *menu_audio_create(scene *s) {
    ...
    menu_attach(menu,
                textselector_create_bind_opts(&tconf, "MONO", NULL, NULL,
                                              &settings_get()->sound.music_mono, mono_opts, 2));

    local->lib_selector =
        textselector_create(&tconf, "PLAYBACK:", menu_audio_library_toggled, local);
    module_source *sources = music_get_module_sources();
    ...
    local->freq_selector =
        textselector_create(&tconf, "FREQUENCY:", menu_audio_freq_toggled, local);
    menu_audio_reset_freqs(local, 1);
    menu_attach(menu, local->freq_selector);

    local->resampler_selector =
        textselector_create(&tconf, "RESAMPLE:", menu_audio_resampler_toggled, local);
