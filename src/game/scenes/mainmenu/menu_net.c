...
component* menu_net_create(scene *s) {
    ...
    component *connect = textbutton_create(&font_large, "CONNECT TO SERVER", COM_ENABLED, menu_net_connect, s);
    widget_set_id(connect, NETWORK_CONNECT_BUTTON_ID);
    menu_attach(menu, connect);
    component *listen = textbutton_create(&font_large, "START SERVER", COM_ENABLED, menu_net_listen, s);
