...
component *menu_video_confirm_create(scene *s, settings_video *old_settings) {
    ...
    menu_attach(menu, filler_create());
    menu_attach(menu, textbutton_create(FONT_BIG, "OK", NULL, COM_ENABLED, video_confirm_ok_clicked, local));
    menu_attach(menu, textbutton_create(FONT_BIG, "CANCEL", NULL, COM_ENABLED, video_confirm_cancel_clicked, local));
