...
guiframe *menu_keyboard_create(scene *s, int selected_player) {
    ...
    for(int i = 0; i < 10; i++) {
        local->keys[i] = textbutton_create(FONT_BIG, "", NULL, COM_ENABLED, menu_keyboard_set_key,
                                           (void *)menu_get_key(local->selected_player, i));
        menu_attach(menu, local->keys[i]);
    }
    menu_attach(menu,
                textbutton_create(FONT_BIG, "DONE", "Leave custom keyboard setup.", COM_ENABLED, menu_keyboard_done, s));
