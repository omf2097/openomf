...
component *menu_language_create(scene *s) {
    ...
    while((filename = (char *)list_iter_next(&it))) {
        // Get localized language name from the .LNG file's first entry
        str_format(&filename2, "%s%s", dirname, filename);
        sd_language lang;
        if(sd_language_create(&lang) != SD_SUCCESS) {
            continue;
        }
        if(sd_language_load(&lang, str_c(&filename2))) {
            INFO("Warning: Unable to load language file '%s'!", str_c(&filename2));
