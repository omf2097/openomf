...
void arena_dynamic_tick(scene *scene, int paused) {
    ...
    if(!paused) {
        ...
        for(int i = 0; i < 2; i++) {
            ...
            progressbar_set_progress(local->health_bars[i], hp * 100, gs->warp_speed || gs->clone ? false : true);
            progressbar_set_progress(local->endurance_bars[i], en * 100, gs->warp_speed || gs->clone ? false : true);
