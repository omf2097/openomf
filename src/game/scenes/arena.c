...
void arena_dynamic_tick(scene *scene, int paused) {
    ...
    if(!paused) {
        ...
        assert(obj_har[0]->pos.fx >= ARENA_LEFT_WALLF && obj_har[0]->pos.fx <= ARENA_RIGHT_WALLF);
        assert(obj_har[1]->pos.fx >= ARENA_LEFT_WALLF && obj_har[1]->pos.fx <= ARENA_RIGHT_WALLF);
        if(hars[0]->health == 0) {
...
static void arena_debug(scene *scene) {
    ...
    for(int i = 0; i < 2; i++) {
        ...
        snprintf(buf, sizeof(buf), "%.2f", HAR_ENDURANCE_TOFLOAT(hars[i]->endurance));
        if(i == 0) {
