...
void cb_vs_spawn_object(object *parent, int id, vec2i pos, vec2f vel, uint8_t flags, int s, int g,
                        void *userdata);
...
void vs_render(scene *scene) {
    ...
    font_render_shadowed(&font_small, local->vs_str,
                         160 - ((strlen(local->vs_str) * font_small.w) / 2), 3, COLOR_YELLOW,
    ...
    if (player2->selectable) {
        ...
        font_render_wrapped(&font_small,
                            lang_get(749 + (11 * player1->pilot_id) + player2->pilot_id), 59, 160,
                            150, COLOR_YELLOW);
        font_render_wrapped(&font_small,
                            lang_get(870 + (11 * player2->pilot_id) + player1->pilot_id),
...
int vs_create(scene *scene) {
    ...
    snprintf(local->vs_str, 128, "%*.*s VS. %*.*s", (int)strlen(pilot1) - 1,
             (int)strlen(pilot1) - 1, pilot1, (int)strlen(pilot2) - 1, (int)strlen(pilot2) - 1,
             pilot2);
    ...
    // Quit Dialog
    dialog_create(&local->quit_dialog, DIALOG_STYLE_YES_NO,
                  "ARE YOU SURE YOU WANT TO QUIT THIS GAME?", 72, 60);
