...
// Somewhat hacky, but easier than trying to figure out the tags (which only work for this scene
// anyway)
struct {
    int anim_no;
    int frame_no;
    ...
    {20, 2, 1},  {21, 1, 1},  {21, 4, 0},  {21, 6, 1}, {21, 9, 0}, {21, 11, 1}, {21, 14, 0},
    {21, 16, 1}, {21, 19, 0}, {20, 5, 0},  {22, 2, 1}, {23, 1, 1}, {23, 4, 0},  {23, 6, 1},
    {23, 9, 0},  {23, 11, 1}, {23, 14, 0}, {22, 5, 0}, {25, 1, 1}, {24, 3, 1},  {25, 4, 0},
    {24, 6, 0},  {27, 1, 1},  {26, 3, 1},  {27, 4, 0}, {26, 6, 0}, {0, 0, 0} // guard
};
...
void player_free(object *obj) { sd_script_free(&obj->animation_state.parser); }
...
void player_reload_with_str(object *obj, const char *custom_str) {
    ...
    if (ret != SD_SUCCESS) {
        PERROR("Decoder error %s at position %d in string \"%s\"", sd_get_error(ret), err_pos,
               custom_str);
    }
...
/*
 * Try to spread <delay> ticks over the 'startup' frames; those that don't spawn projectiles or have
 * hit coordinates
 */
...
void player_describe_object(object *obj) {
    ...
    if (obj->cur_sprite) {
        ...
        DEBUG("CURRENT = %d - %d + %d - %d", obj->pos.y, obj->cur_sprite->pos.y,
              rstate->o_correction.y, obj->cur_sprite->data->h);
    }
}
#endif /* DEBUGMODE */
...
void player_run(object *obj) {
    ...
    state->entered_frame =
        sd_script_frame_changed(&state->parser, state->previous_tick, state->current_tick);
    if (state->entered_frame) {
        ...
        /*if (sd_script_isset(frame, "bm")) {
            if (sd_script_isset(frame, "am") && sd_script_isset(frame, "e")) {
                ...
                DEBUG("BE tag with x/y offsets: %d %d %d %d", trans_x, trans_y,
        object_get_direction(obj), object_get_direction(state->enemy)); DEBUG("enemy x %d modified
        trans_x: %d (%d * %d * %d)", state->enemy->pos.x, (trans_x * object_get_direction(obj) *
        object_get_direction(state->enemy)), trans_x, object_get_direction(obj),
    ...
    if (trans_x || trans_y) {
        if (sd_script_isset(frame, "v")) {
            ...
            obj->vel.y = trans_y;
            // DEBUG("vel x+%d, y+%d to x=%f, y=%f", trans_x * (mp & 0x20 ? -1 : 1), trans_y,
            // obj->vel.x, obj->vel.y);
        } else {
            obj->pos.x += trans_x * (mp & 0x20 ? -1 : 1);
            obj->pos.y += trans_y;
            // DEBUG("pos x+%d, y+%d to x=%f, y=%f", trans_x * (mp & 0x20 ? -1 : 1), trans_y,
            // obj->pos.x, obj->pos.y);
        }
    }
    ...
    if (state->entered_frame) {
        ...
        if (sd_script_isset(frame, "m") && state->spawn != NULL) {
            ...
            state->spawn(obj, sd_script_get(frame, "m"), vec2i_create(mx, my), vec2f_create(vx, vy),
        ...
        if (sd_script_isset(frame, "bu") && obj->vel.y < 0.0f) {
            ...
            // assume that bu is used in conjunction with 'vy-X' and that we want to land in the
            // center of the arena
...
void player_set_repeat(object *obj, int repeat) { obj->animation_state.repeat = repeat; }

int player_get_repeat(const object *obj) { return obj->animation_state.repeat; }
...
int player_get_current_tick(const object *obj) { return obj->animation_state.current_tick; }
