...
void cb_scene_spawn_object(object *parent, int id, vec2i pos, vec2f vel, uint8_t flags, int s,
                           int g, void *userdata);
...
int scene_create(scene *scene, game_state *gs, int scene_id) {
    ...
    if (load_bk_file(&scene->bk_data, resource_id)) {
        PERROR("Unable to load scene %s (%s)!", scene_get_name(scene_id),
...
void scene_set_userdata(scene *scene, void *userdata) { scene->userdata = userdata; }

void *scene_get_userdata(scene *scene) { return scene->userdata; }
...
void scene_set_free_cb(scene *scene, scene_free_cb cbfunc) { scene->free = cbfunc; }

void scene_set_event_cb(scene *scene, scene_event_cb cbfunc) { scene->event = cbfunc; }

void scene_set_render_cb(scene *scene, scene_render_cb cbfunc) { scene->render = cbfunc; }

void scene_set_render_overlay_cb(scene *scene, scene_render_overlay_cb cbfunc) {
    scene->render_overlay = cbfunc;
}

void scene_set_startup_cb(scene *scene, scene_startup_cb cbfunc) { scene->startup = cbfunc; }

void scene_set_dynamic_tick_cb(scene *scene, scene_tick_cb cbfunc) { scene->dynamic_tick = cbfunc; }

void scene_set_static_tick_cb(scene *scene, scene_tick_cb cbfunc) { scene->static_tick = cbfunc; }
