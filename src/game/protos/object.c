...
void object_apply_controllable_velocity(object *obj, object *obj_har, char input) {
    if(player_frame_isset(obj, "cx")) {
        fixedpt cx =
            fixedpt_xmul(fixedpt_fromint(player_frame_get(obj, "cx")) / 10, obj_har->horizontal_velocity_modifierf);
        if(input == '4') {
        ...
        if(player_frame_isset(obj, "cy")) {
            fixedpt cy =
                fixedpt_xmul(fixedpt_fromint(player_frame_get(obj, "cy")) / 10, obj_har->vertical_velocity_modifierf);
            if(input == '8') {
