...
int object_serialize(object *obj, serial *ser) {
    ...
    /*DEBUG("Animation state: [%d] %s, ticks = %d stride = %d direction = %d pos = %f,%f vel = %f,%f
     * gravity = %f", strlen(player_get_str(obj))+1, player_get_str(obj),
     * obj->animation_state.ticks, obj->stride, obj->animation_state.reverse, obj->pos.x,
     * obj->pos.y, obj->vel.x, obj->vel.y, obj->gravity);*/
...
void object_set_delay(object *obj, int delay) { player_set_delay(obj, delay); }
...
void object_set_effects(object *obj, int effects) { obj->video_effects = effects; }

int object_get_effects(const object *obj) { return obj->video_effects; }

void object_add_effects(object *obj, int effects) { obj->video_effects |= effects; }

void object_del_effects(object *obj, int effects) { obj->video_effects &= ~effects; }
...
void object_render(object *obj) {
    ...
    // Render
    video_render_sprite_flip_scale_opacity_tint(obj->cur_surface, x, y, rstate->blendmode,
                                                obj->pal_offset, flipmode, obj->y_percent, opacity,
                                                tint);
}
...
void object_render_shadow(object *obj) {
    ...
    if (object_get_direction(obj) == OBJECT_FACE_LEFT) {
        x = (obj->pos.x + obj->sprite_state.o_correction.x) - obj->cur_sprite->pos.x -
            object_get_size(obj).x;
        flipmode ^= FLIP_HORIZONTAL;
    }
    ...
    for (int i = 0; i < 2; i++) {
        video_render_sprite_flip_scale_opacity_tint(obj->cur_sprite->data, x + i, y + i,
                                                    BLEND_ALPHA, obj->pal_offset, flipmode, scale_y,
                                                    65, color_create(0, 0, 0, 255));
    }
}
...
int object_scenewide_palette_transform(object *obj, screen_palette *pal) {
    ...
    if (rstate->pal_entry_count > 0 && rstate->duration > 0) {
        float bp =
            ((float)rstate->pal_begin) + ((float)rstate->pal_end - (float)rstate->pal_begin) *
        ...
        float u;
        float k = bp / 255.0f;
        for (int i = rstate->pal_start_index; i < rstate->pal_start_index + rstate->pal_entry_count;
             i++) {
            if (rstate->pal_tint) {
                m = max3(pal->data[i][0], pal->data[i][1], pal->data[i][2]);
                u = m / 255.0f;
                pal->data[i][0] =
                    max2(0, min2(255, pal->data[i][0] + u * k * (b.r - pal->data[i][0])));
                pal->data[i][1] =
                    max2(0, min2(255, pal->data[i][1] + u * k * (b.g - pal->data[i][1])));
                pal->data[i][2] =
                    max2(0, min2(255, pal->data[i][2] + u * k * (b.b - pal->data[i][2])));
            } else {
...
/** Frees the object and all resources attached to it (even the animation, if it is owned by the
 * object) \param obj Object handle
 */
 ...
 */
void object_set_stl(object *obj, char *ptr) { obj->sound_translation_table = ptr; }

/** Returns a pointer to the sound translation table (30 byte char array)
 * \param obj Object handle
 * \return Pointer to the sound translation table
 */
char *object_get_stl(const object *obj) { return obj->sound_translation_table; }

/** Sets the owner of the animation. If OWNER_OBJECT, the object will free animation on
 * object_free(). \param obj Object handle \param owner Owner of the object (OWNER_EXTERNAL,
 * OWNER_OBJECT)
 */
void object_set_animation_owner(object *obj, int owner) { obj->cur_animation_own = owner; }
 ...
 */
animation *object_get_animation(object *obj) { return obj->cur_animation; }
 ...
 */
void object_set_sprite_override(object *obj, int override) { obj->sprite_override = override; }
...
int object_is_rewind_tag_disabled(const object *obj) { return obj->animation_state.disable_d; }

uint32_t object_get_age(object *obj) { return obj->age; }
...
int object_is_airborne(const object *obj) { return obj->pos.y < ARENA_FLOOR; }

/* Attaches one object to another. Positions are synced to this from the attached. */
void object_attach_to(object *obj, const object *attach_to) { obj->attached_to = attach_to; }
