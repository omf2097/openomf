...
int intersect_object_object(object *a, object *b) {
    ...
    return !(pos_a.x > (pos_b.x + size_b.x) || pos_a.y > (pos_b.y + size_b.y) ||
             (pos_a.x + size_a.x) < pos_b.x || (pos_a.y + size_a.y) < pos_b.y);
}
...
int intersect_object_point(object *obj, vec2i point) {
    ...
    return (point.x < (pos.x + size.x) && point.y < (pos.y + size.y) && point.x > pos.x &&
            point.y > pos.y);
}
...
int intersect_sprite_hitpoint(object *obj, object *target, int level, vec2i *point) {
    ...
    while ((cc = iter_next(&it)) != NULL) {
        ...
        // convert global coordinates to local coordinates by compensating for the other player's
        // position
