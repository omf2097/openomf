...
void test_find_next_line_end(void) {
    ...
    str_from_c(&s, "ABBABB");
    CU_ASSERT_EQUAL(find_next_line_end(&s, &f, TEXT_ROW_HORIZONTAL, 0, 0, 16), 3);
    str_free(&s);

    // Same as above, but in vertical direction,
    str_from_c(&s, "ABBABB");
    CU_ASSERT_EQUAL(find_next_line_end(&s, &f, TEXT_ROW_VERTICAL, 0, 0, 16), 3);
    str_free(&s);

    // No Letter spacing should make this just barely fit.
    str_from_c(&s, "AA");
    CU_ASSERT_EQUAL(find_next_line_end(&s, &f, TEXT_ROW_HORIZONTAL, 0, 0, 16), 2);
    str_free(&s);

    // One pixel letter spacing should make this not fit.
    str_from_c(&s, "AA");
    CU_ASSERT_EQUAL(find_next_line_end(&s, &f, TEXT_ROW_HORIZONTAL, 0, 1, 16), 1);
    str_free(&s);

    // Letter does not fit, so just return 0
    str_from_c(&s, "A");
    CU_ASSERT_EQUAL(find_next_line_end(&s, &f, TEXT_ROW_HORIZONTAL, 0, 0, 7), 0);
    str_free(&s);
    ...
    str_from_c(&s, "A BBBB");
    CU_ASSERT_EQUAL(find_next_line_end(&s, &f, TEXT_ROW_HORIZONTAL, 2, 0, 16), 6);
    str_free(&s);

    // Empty string should just return 0
    str_from_c(&s, "");
    CU_ASSERT_EQUAL(find_next_line_end(&s, &f, TEXT_ROW_HORIZONTAL, 0, 0, 16), 0);
    str_free(&s);
