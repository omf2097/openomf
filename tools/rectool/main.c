    ...
    if (rec != NULL) {
        ...
        for (int i = 0; i < rec->move_count; i++) {
            ...
            printf("%6u %10u %5u %6u %6u %6u %22s", i, rec->moves[i].tick, rec->moves[i].lookup_id,
                   rec->moves[i].player_id, rec->moves[i].raw_action,
                   sd_rec_extra_len(rec->moves[i].lookup_id), tmp);

            if (rec->moves[i].lookup_id > 2) {
                print_bytes(rec->moves[i].extra_data, sd_rec_extra_len(rec->moves[i].lookup_id) - 1,
                            8, 2);
            }
            printf("\n");
        }
...
int main(int argc, char *argv[]) {
    ...
    struct arg_int *delete =
        arg_intn("d", "delete", "<number>", 0, 10, "Delete an existing element");
