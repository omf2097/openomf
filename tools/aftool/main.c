...
void sprite_play(sd_af_file *af, sd_bk_file *bk, int scale, int anim, int sprite) {
    ...
    SDL_Window *window =
        SDL_CreateWindow("OMF2097 Remake", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                         320 * scale, 200 * scale, SDL_WINDOW_SHOWN | SDL_WINDOW_OPENGL);
    ...
    if ((rendertarget = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888,
                                          SDL_TEXTUREACCESS_TARGET, 320, 200)) == 0) {
    ...
    if (!(surface = SDL_CreateRGBSurfaceFrom((void *)img.data, img.w, img.h, 32, img.w * 4, rmask,
                                             gmask, bmask, amask))) {
        ...
        if (SDL_PollEvent(&e)) {
            if (e.type == SDL_QUIT) {
                ...
                if (changed) {
                    ...
                    sd_sprite_rgba_decode(&img, s, bk->palettes[0], -1);
                    printf("Sprite Info: pos=(%d,%d) size=(%d,%d) len=%d\n", s->pos_x, s->pos_y,
                           s->width, s->height, s->len);

                    if (!(surface =
                              SDL_CreateRGBSurfaceFrom((void *)img.data, img.w, img.h, 32,
                                                       img.w * 4, rmask, gmask, bmask, amask))) {
                        printf("Could not create surface: %s\n", SDL_GetError());
                        return;
                    }
...
int move_key_get_id(const char *key) {
    ...
    if (strcmp(key, "category") == 0)
        return 42;
    if (strcmp(key, "unknown_14") == 0)
        return 43;
    if (strcmp(key, "scrap_amount") == 0)
        return 44;
...
void move_set_key(sd_move *move, sd_animation *ani, const char **key, int kcount,
                  const char *value) {
    int tmp = 0;
    ...
    case 30:
        move->unknown_0 = conv_uword(value);
        break;
    case 31:
        move->unknown_2 = conv_uword(value);
        break;
    case 33:
        move->unknown_4 = conv_ubyte(value);
        break;
    case 34:
        move->unknown_5 = conv_ubyte(value);
        break;
    case 35:
        move->unknown_6 = conv_ubyte(value);
        break;
    case 36:
        move->unknown_7 = conv_ubyte(value);
        break;
    case 37:
        move->unknown_8 = conv_ubyte(value);
        break;
    case 38:
        move->unknown_9 = conv_ubyte(value);
        break;
    case 39:
        move->unknown_10 = conv_ubyte(value);
        break;
    case 40:
    ...
    case 42:
        ...
        break;
    case 43:
        move->unknown_14 = conv_ubyte(value);
        break;
    case 44:
        move->scrap_amount = conv_ubyte(value);
        break;
    case 45:
        move->successor_id = conv_ubyte(value);
        break;
    case 46:
        move->damage_amount = conv_ubyte(value);
        break;
    case 47:
        move->unknown_18 = conv_ubyte(value);
        break;
    case 48:
        move->unknown_19 = conv_ubyte(value);
        break;
    case 49:
        move->points = conv_ubyte(value);
        break;
    ...
    case 15:
        ...
        if (tmp < 21) {
            ...
            printf(
                "String is too long! (%u bytes) Maximum size for move_string is 21 characters!\n",
                tmp);
            return;
        }
        break;
...
void af_set_key(sd_af_file *af, const char **key, int kcount, const char *value) {
    ...
    case 4:
        ...
        break;
    case 5:
        af->forward_speed = conv_float(value);
        break;
    case 6:
        af->reverse_speed = conv_float(value);
        break;
    case 7:
        af->jump_speed = conv_float(value);
        break;
    case 8:
...
void move_strip_key(sd_move *move, sd_animation *ani, const char **key, int kcount,
                    const char *tag) {
    int kn = move_key_get_id(key[0]);
    switch (kn) {
    case 16:
...
int main(int argc, char *argv[]) {
    ...
    struct arg_int *sprite =
        arg_int0("s", "sprite", "<sprite_id>", "Select sprite (requires --move)");
    ...
    struct arg_str *strip =
        arg_str0(NULL, "strip", "<tag>", "Strip tag from selected file, move or key (string)");
    ...
    struct arg_lit *play =
        arg_lit0(NULL, "play", "Play animation or sprite (requires --anim and --palette)");
