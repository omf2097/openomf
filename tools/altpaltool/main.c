...
int main(int argc, char *argv[]) {
    ...
    struct arg_file *export =
        arg_file0("e", "export", "<file>", "Export selected palette to GPL file");
    struct arg_file *import =
        arg_file0("i", "import", "<file>", "Import selected palette from GPL file");
    ...
    if (export->count > 0) {
        ...
        if (ret == SD_SUCCESS) {
            ...
            printf("Error while attempting to save palette %d to file %s: %s", pal_id,
        ...
        if (ret == SD_SUCCESS) {
            ...
            printf("Error while attempting to load palette %d from file %s: %s", pal_id,
                   import->filename[0], sd_get_error(ret));
        }
    }
