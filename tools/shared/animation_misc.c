...
void sprite_export_key(sd_sprite *s, const char **key, int kcount, const char *filename,
                       sd_bk_file *bk) {
    switch (sprite_key_get_id(key[0])) {
    case 1:
    case 2:
...
void anim_common_info(sd_animation *ani) {
    ...
    for (int i = 0; i < ani->coord_count; i++) {
        printf("   - x,y = (%d,%d), null = %d, frame_id = %d\n", ani->coord_table[i].x,
               ani->coord_table[i].y, ani->coord_table[i].null, ani->coord_table[i].frame_id);
    }
    printf(" * Sprites:          %d\n", ani->sprite_count);
        ...
        if (kcount == 2) {
            ...
            if (tmp < ani->coord_count) {
                printf("x,y = (%d,%d), null = %d, frame_id = %d\n", ani->coord_table[tmp].x,
                       ani->coord_table[tmp].y, ani->coord_table[tmp].null,
            ...
            for (int i = 0; i < ani->coord_count; i++) {
                printf("x,y = (%d,%d), null = %d, frame_id = %d\n", ani->coord_table[i].x,
                       ani->coord_table[i].y, ani->coord_table[i].null,
