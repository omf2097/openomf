...
int main(int argc, char *argv[]) {
    ...
    struct arg_file *export =
        arg_file0("e", "export", "<file>", "Export selected photo sprite to ppm file");
    ...
    if (entry->count > 0) {
        ...
        for (int i = 0; i < pic.photo_count; i++) {
            ...
            printf("%3d %8s %7d %7d %4d %4d %4d %4d %4d\n", i, (photo->sex ? "FEMALE" : "MALE"),
                   photo->is_player, photo->sprite->len, photo->sprite->width,
                   photo->sprite->height, photo->sprite->pos_x, photo->sprite->pos_y,
                   photo->unk_flag);
        }
    }
